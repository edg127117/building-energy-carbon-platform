...
- 历史决策：从 [`docs/superpowers/README.md`](docs/superpowers/README.md) 定位既有设计和计划，不能把历史状态当作当前事实，也不再默认向该目录新增任务文档。
...
PR 标题统一使用 `type(scope): 中文动宾短语`，冒号必须为半角并带一个空格。`type` 允许 `feat`、`fix`、`docs`、`chore`、`refactor`、`test`、`build`、`ci`、`perf`；`scope` 使用小写英文、数字或 `-`、`_`、`.`、`/`，且必须准确反映改动范围。标题中的类型、范围和描述必须与实际改动一致。

新模块、新 API、数据库结构、权限、缓存、并发、数据流程、跨子系统功能或大范围架构变化，必须先确认设计和实施范围；是否持久化设计按第 6 节判断。纯文档、注释、拼写和不改变行为的简单格式调整可以跳过独立设计，但不能跳过范围、验证和 Git 流程。
...
- `docs/designs`：按需保存正式版本、重大架构或权威需求变化的正式设计；
...
普通任务的实施步骤保留在当前 Codex 任务、Issue 或 PR 中，不新建永久计划文档。大型跨会话模块仅在交接或上下文丢失风险确实存在时，在 `PROJECT_STATUS.md` 保存目标、范围、当前阶段、已完成、待完成、风险和验收入口；重大且需要长期追溯的技术决策写入 `docs/adr`，新正式版本、重大架构或权威需求变化写入 `docs/designs`。目录在首次确有内容时创建，不为占位提前生成文件。
