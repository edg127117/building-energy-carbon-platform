...
CREATE STABLE IF NOT EXISTS `iot_telemetry`.`st_raw_event` (
    `ts`                TIMESTAMP NOT NULL COMMENT '设备采集时间',
    `received_time`     TIMESTAMP NOT NULL COMMENT '服务器接收时间',
    `val`               DOUBLE NOT NULL,
    `data_quality`      TINYINT NOT NULL DEFAULT 0,
    `late_flag`         TINYINT NOT NULL DEFAULT 0,
    `source_system`     NCHAR(50) NOT NULL,
    `source_point_code` NCHAR(100) NOT NULL,
    `source_device_id`  NCHAR(50) NOT NULL
) TAGS (
    `point_id`       NCHAR(32) NOT NULL,
    `point_code`     NCHAR(100) NOT NULL,
    `building_id`    NCHAR(32) NOT NULL,
    `system_group_id` NCHAR(32),
    `equip_id`       NCHAR(32),
    `equip_code`     NCHAR(50),
    `family_code`    NCHAR(20) NOT NULL,
    `component_code` NCHAR(20),
    `suffix_code`    NCHAR(20) NOT NULL,
    `is_for_calc`    TINYINT NOT NULL DEFAULT 0
);
