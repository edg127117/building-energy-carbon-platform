...
修复 Redis 未命中时的 TDengine 批量最新值查询，使每个完整指标身份返回一条
最新成功记录和一条最新异常记录。当前数据模型保证指标 ID 与完整身份一一对应；
若后续发生身份标签漂移，Service 仍按指标 ID 合并结果。现有权限、DTO、缓存和
公式逻辑保持不变。
...
同一指标、同一来源分钟使用相同 TDengine 主时间戳；异常重试只会在异常子表内
覆盖该分钟旧异常，因此最新主时间戳对应最终保留的异常记录，不需要再按
`calculated_at` 二次截断。成功表与异常表相互独立，同一分钟若两边都有记录，
由现有 Service 与 Redis 的 `SUCCESS` 优先规则决定接口最终状态，不发生跨表
物理覆盖或删除。
