...
> **文档状态：调度隔离与嵌套分钟锁修复已通过自动化验证；待专用本地端到端复验**
>
...
4. 迟到修正持有单个“测点 + 分钟”锁时继续同步发布 READY 并执行插值；下游再次申请
   多个分钟锁。多个线程分别持有不同外层锁后会循环等待，现场线程转储已确认 Java 级死锁。
...
迟到修正的外层锁只覆盖当前测点分钟的“读取现状、读取原始证据、重聚合、Q0 条件写入”。
Q0 写入完成后必须先释放外层锁，再执行替换计数、READY 发布、公式重算和插值回溯。
这样仓储批量写仍按条带序号统一取锁，但调用链不会持有一个条带再申请另一组条带。

锁外下游可能出现短暂重复或乱序，由现有 `finalizedAt` 水位、指标幂等写入和低频恢复收口；
这些是可恢复副作用，不能为了串行下游重新扩大外层锁范围。
...
- 两个迟到修正并发进入下游批量锁时都能在超时内结束，不存在循环等待。
...
该标记只代表本地端到端联调，不代表现场验收。修复死锁后的本地验收还必须使用
`-MinimumTrendPoints 10`，确认 10/10 趋势点匹配、人工任务最终为 `SUCCEEDED`，并通过
`jcmd <PID> Thread.print` 确认没有 Java-level deadlock。
