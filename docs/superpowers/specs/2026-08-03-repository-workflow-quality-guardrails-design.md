...
- 文件范围检查；
- 测试命令、结果、跳过项和未验证内容；
- 注释检查，包括涉及类/组件、关键方法、调用链和有意不加注释的简单方法。
...
- 生产代码发生变化时，要求 PR 的“注释检查”明确列出类/组件、关键方法、调用链和有意排除项。

自动检查只处理新增文件和新增注释，不要求一次修复全部旧代码，避免把历史欠账强塞进任意功能 PR。
