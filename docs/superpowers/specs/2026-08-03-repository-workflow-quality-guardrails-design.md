...
> [历史任务目录](../README.md)、[固定工作规则](../../../AGENTS.md)、[项目指南](../../../PROJECT_GUIDE.md)、
...
- 提取变化文件中的全部现有注释，扫描任务历史、已经失效的未来承诺和无信息代码复述，不只扫描本次新增行；
- 生成固定 Markdown 表格，要求每个符号填写 `关键-已新增说明`、`关键-已更新说明`、`关键-现有说明已核验` 或 `简单-无需说明`；
- 对 `简单-无需说明` 强制填写简短原因，不能只写“无需注释”；
- 为每个文件保留类/组件职责、上游、下游、数据源、最终消费者和“现有注释时效已核验”字段。
...
- 检查变化文件全部现有注释中的任务历史、高风险未来承诺和无信息代码复述；
...
结构覆盖和注释文本检查都处理所有发生变化的生产文件。任务历史模式包括“当前任务”“本次修改”“本 PR”“本次修复”等；未来承诺只匹配“以后、后续、未来”等时间词与“支持、实现、优化、补充、新增、完成”等承诺动作的组合，避免把“后续节点”“当前温度”等业务含义误判。功能 PR 不要求修复未变化文件的历史欠账，但只要修改了某个生产文件，就必须完整审查该文件的现有类级说明、全部方法和全部现有注释。已经实现的目标必须改写为当前真实行为，已经取消或过期的承诺必须删除。
...
- 标记为简单方法时是否填写了具体排除原因；
- 每个变化文件是否填写职责、上下游、数据源、结果消费者和现有注释时效核验结果；
- 新增生产 Java 类完全没有类级 Javadoc；
- 新增核心前端文件完全没有业务说明；
- 变化文件的任一现有注释包含“当前任务”“本次修改”“以后支持”“后续支持”等任务历史或未来承诺；
- 变化文件的任一现有注释只写“获取数据”“进行判断”“循环处理”“返回结果”等无信息复述；
...
- 注释是否足以让业务阅读者理解流程；
- 未命中机械规则的现有注释是否仍与当前代码行为一致，是否存在已经实现、取消或过期但未更新的描述；
- 哪些简单方法不需要注释；
...
- 变化文件在未修改行中保留“后续支持某功能”等过期承诺时失败；
- 变化文件保留任务历史或无信息复述注释时失败；
- “后续节点”“当前温度”等真实业务语义不会被未来承诺规则误判；
- PR 缺少必填章节或注释证据失败；
...
- 变化文件中的历史注释与新增注释采用相同标准；存在过期任务描述、已实现但未更新的未来承诺或无信息复述时检查失败。
