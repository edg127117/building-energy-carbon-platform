...
桌面与窄屏完整截图确认温度单位可见，36℃ 对应自动扩展到 40℃ 的刻度，0℃ 保持真实读数。目录同步和运行统计任务状态已按完整状态集合显示中文，未知目录状态显示“同步状态待确认”；目录同步默认摘要改为中文任务与处理结果，提交成功后清空数据源技术标识，完整数据源标识、任务编号和失败代码收进折叠的同步技术详情，不再直接暴露后端枚举和长编号。
