...
本文记录第二阶段前端可视化已经讨论并确认的实施基线。公共骨架及已确认的继承业务能力已形成
**活动实现候选**；这不代表阶段二业务页面内容已经全部确认，也不代表真实后端、设备或现场大屏验收完成。
...
实现候选已将新骨架设为唯一前端入口，并迁入登录、权限后台、资产与设备接入以及继承 HVAC
查询能力；旧深色页面、旧入口、旧依赖和旧样式不再保留。继承的 19 测点与四项指标只是可替换
的现有业务能力，不属于通用骨架约束，后续可按已确认业务契约调整或移除。
