...
> 文档状态：待评审设计，不代表已实现或已部署。  
> 用户已认可方向：自动接入与人工选择模板同时保留，支持后续不同厂家的空调，减少重复操作。  
> 本次交付：设计与实施计划。详细方案确认后，才修改业务代码、数据库及测试环境。  
...
本文件完成后，先确认第 1、3、4、7 节的目标和边界，再进入阶段 1。需要新增的权限、契约和数据结构属于本功能的一部分，须与设计一致；实施中若发现必须改变审批、存量产品归属或质量门禁，应先说明差异并修订设计，不能自行扩大范围。
