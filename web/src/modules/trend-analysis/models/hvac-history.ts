...
export function validateHistoryRange(
  ...
  if (!Number.isFinite(from) || !Number.isFinite(to)) {
    return copy('validation.invalid')
  }
  if (from >= to) {
    return copy('validation.order')
  }
  if (to - from > MAX_HISTORY_SPAN_MS) {
    return copy('validation.span')
  }
  return null
}
...
export function historyQualityLabel(quality: number | null): string {
  if (quality === 0) return copy('quality.q0')
  if (quality === 1) return copy('quality.q1')
  if (quality === 2) return copy('quality.q2')
  if (quality === null) return copy('quality.none')
  return copy('quality.unknown', { quality })
}
