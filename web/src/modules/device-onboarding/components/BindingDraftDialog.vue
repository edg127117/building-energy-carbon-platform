...
}>(), { pending: null, product: null, productLoading: false, productTotal: 0, productPage: 1, productSize: 20, namingRules: () => [], namingRulesLoading: false, submitting: false, allowEmptyPoints: false, productSearchEnabled: true, numericSources: () => [], numericSourcesLoading: false, bindingOptions: null })
...
function reset() {
  Object.assign(form, {
    ...
    mode: 'existing', existingEquipmentId: undefined, equipmentName: '', manufacturer: '', pointCodePrefix: '', numericSourceId: undefined, bindings: {},
  })
...
<template>
  <ElDialog :model-value="open" :title="t('deviceOnboarding.pending.binding')" width="80%" @update:model-value="emit('close')">
    <ElForm label-position="top" class="binding-form" @submit.prevent="submit">
      ...
      <ElFormItem v-if="allowEmptyPoints && hasIncludedPoints" :label="t('deviceOnboarding.labels.numericSource')" required><ElSelect v-model="form.numericSourceId" class="wide-control" :loading="numericSourcesLoading"><ElOption v-for="item in numericSources" :key="item.sourceId" :label="`${item.sourceName} · ${item.sourceCode}`" :value="item.sourceId" /></ElSelect></ElFormItem>
