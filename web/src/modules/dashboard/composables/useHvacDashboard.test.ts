...
import type { HvacRealtimeDisconnect } from '@/modules/dashboard/realtime/hvac-realtime-protocol'
import type { HvacRealtimeIndicator } from '@/modules/dashboard/models/hvac'
...
describe('useHvacDashboard', () => {
  ...
  it('keeps successful snapshot data when indicators fail', async () => {
    ...
    expect(dashboard.indicatorError.value).toBe('请求失败，请稍后重试。')
  })
  ...
  it('keeps the previous successful section after a later refresh fails', async () => {
    ...
    expect(dashboard.snapshot.value).toEqual(snapshot)
    expect(dashboard.snapshotError.value).toBe('请求失败，请稍后重试。')
  })
