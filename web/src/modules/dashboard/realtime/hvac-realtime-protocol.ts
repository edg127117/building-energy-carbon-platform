...
function record(value: unknown, description: string): Record<string, unknown> {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new HvacRealtimeProtocolError(`${description}:invalid-record`)
  }
...
function text(value: unknown, field: string): string {
  if (typeof value !== 'string' || !value.trim()) {
    throw new HvacRealtimeProtocolError(`${field}:missing-or-invalid`)
  }
  return value
}
...
function finiteNumber(value: unknown, field: string): number {
  if (typeof value !== 'number' || !Number.isFinite(value)) {
    throw new HvacRealtimeProtocolError(`${field}:not-finite`)
  }
  return value
}
...
function textArray(value: unknown, field: string): string[] {
  if (!Array.isArray(value) || value.some((item) => typeof item !== 'string')) {
    throw new HvacRealtimeProtocolError(`${field}:not-string-array`)
  }
  return [...value]
}
...
function indicatorCode(value: unknown): HvacRealtimeIndicatorCode {
  ...
  if (!(HVAC_REALTIME_INDICATOR_CODES as readonly string[]).includes(code)) {
    throw new HvacRealtimeProtocolError('indicatorCode:out-of-scope')
  }
...
export function parseHvacRealtimeServerMessage(
  ...
  try {
    ...
    throw new HvacRealtimeProtocolError('message:invalid-json')
  }

  const message = record(decoded, 'message')
  ...
  switch (type) {
    ...
    default:
      throw new HvacRealtimeProtocolError('message:unknown-type')
  }
}
...
export function resolveHvacRealtimeUrl(
  ...
  if (url.protocol === 'http:') url.protocol = 'ws:'
    ...
    throw new HvacRealtimeProtocolError('url:unsupported-protocol')
  }
