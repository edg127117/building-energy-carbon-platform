...
- 最新测点展示尚未区分采集时的数据质量与当前数据新鲜度；历史最后值可能继续显示 `NORMAL` 和 100% 完整率，必须通过独立任务修复，不能由指标卡文案优化掩盖。
...
1. 区分测点数据质量与数据新鲜度，修复过期历史值仍显示 `NORMAL` 和 100% 完整率的问题。
2. 接入真实历史数据展示和时间范围查询，形成可验证的前后端闭环。
3. 接入 `/ws/hvac`，并保留轮询作为明确的重连或降级策略。
4. 建立用户、角色、菜单和建筑授权的最小后台前端入口。
