...
- 活动候选设计：已形成[可配置设备接入与业务绑定设计](docs/designs/2026-08-17-configurable-device-onboarding-design.md)，在不重写现有云端 JSON 适配器和正式时序链的前提下覆盖产品型号、待绑定设备、接入向导以及建筑/设备/测点管理页面。
- 候选身份：当前 Git 版本已完成工作包 A 的领域发现后端、工作包 B 的绑定事务与版本化管理 API、工作包 C 的资产管理页面，以及工作包 D 的产品模板和设备接入页面；A、B 新增结构与 C、D 菜单迁移已通过隔离和目标业务 MySQL 8.0 验证。工作包 E 已完成目标云基础部署、合成单包转换核对和目标业务库迁移，但真实设备与长时间运行仍未验收，候选也尚未批准为新的正式版本。当前权威基线仍是既有已合并 V1 能力。
...
- 云端适配器拥有独立 Maven 构建、H2/Mock 自动化测试、环境变量示例和 MySQL schema，并通过默认仅绑定回环地址的 HTTP 端点暴露 Actuator 健康检查与指标；后端 CI 同时验证本地平台与云端适配器。
...
- 隔离 MySQL 已验证新增状态检查、外键、UTF-8 菜单、既有设备保留和临时迁移过程清理；目标业务库执行证据单独记录在下一节。

### 3.10 工作包 A、B、C、D 的目标业务 MySQL 迁移

- 目标业务库执行前已完成全库备份，并核对基础表、外部身份、产品、模板指标、设备别名、菜单 ID、菜单路径、设备产品列、索引和外键；未发现迁移冲突。
- `13`、`14`、`15`、`16` SQL 已按顺序在目标业务 MySQL 8.0 执行，并在已存在结构上通过幂等重放；迁移临时存储过程均已清理。
- 正式设备、身份、测点和别名的数据快照在迁移前后完全一致；既有设备未批量回填产品，新增产品、模板、待绑定和审计表保持空表。
- 资产与设备接入菜单只分配给 `PLATFORM_ADMIN`，未向其他角色扩权。该迁移证据不等于真实设备、正式入库或长时间运行已经验收。

### 3.11 合成设备接入链路验收

- 隔离 Docker 冒烟已用固定合成标准多字段身份验证未知发现、管理员建产品、绑定新设备与测点、身份启用和停用；绑定但未启用、停用后的报文均不进入 TDengine，启用后的下一包进入绑定的建筑、设备和测点。
- 同一事件重复发布只保留一条原始事件；缺少指标字段和协议不匹配报文均被拒绝，不增加正式时序行。发布端的 QoS 1 结果只视为本地 Broker 接收，验收结论同时以 MySQL 待绑定事实和 TDengine 原始事件回读为准。
- 验收只允许操作 `iot-smoke-*` 容器和固定测试卷，运行后同时清理 MySQL 产品、设备、身份、测点、别名、待绑定与审计记录，以及 TDengine 合成子表，并验证合成主事实和原始事件归零。
- 该结果证明本地合成协议链可重复运行，不等于真实设备 TLS、云端到本地网络连续性、真实字段单位或 24 小时稳定性已经验收。

证据入口：[`Test-DeviceOnboardingSmoke.ps1`](scripts/Test-DeviceOnboardingSmoke.ps1)、[`publish-device-onboarding-message.mjs`](.scripts/publish-device-onboarding-message.mjs)、[`Invoke-CleanHvacSmoke.ps1`](scripts/Invoke-CleanHvacSmoke.ps1)。

## 4. 尚未完成或待继续

- 真实现场设备、网络抖动、长时间运行和现场数据质量仍需独立验收，不能由单元测试或本地冒烟替代。
- 目标云已核验 EMQX 账号认证与 Topic ACL、云端 MySQL、适配器 systemd 进程、回环健康/指标端点和合成单包转换；本地平台合成接入已验证，但真实设备 TLS 连接、正式字段语义、云到本地网络连续性及真实端到端入库仍需现场验证，仓库代码不会自动创建这些外部安全配置。
...
1. 连接真实设备，确认正式单位、累计/周期语义、发送周期、时间戳和序号，并通过新的待绑定与接入流程完成单包核对。
2. 在真实单包和本地正式链确认后，执行 24 小时运行、断线恢复和数据质量验收。
3. 单独设计并补齐第三方 COP、设备运行状态 REST API 与 Swagger 文档。
...
| [`docs/MQTT-硬件数据对接说明.md`](docs/MQTT-硬件数据对接说明.md) | 当前静态契约已核验 | 已对照云端适配、本地预注册、双 Topic 路由、ACK 和重投实现；目标云基础部署和合成单包已核验，真实硬件、TLS 与长时间网络运行仍需现场验收。 |
| [`docs/designs/2026-08-17-configurable-device-onboarding-design.md`](docs/designs/2026-08-17-configurable-device-onboarding-design.md) | 活动候选设计、工作包 A/B/C/D 已实现、E 部分完成 | 已明确既有约束的保留与替代、云端适配边界、本地发现与绑定、页面范围和分段验收；E 已完成目标云基础部署、合成单包和目标业务库迁移，真实硬件与长时间运行仍未验收，候选也未批准为正式基线。 |
