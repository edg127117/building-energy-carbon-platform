...
- 候选身份：当前 Git 版本已完成工作包 A 的领域发现后端、工作包 B 的绑定事务与版本化管理 API、工作包 C 的资产管理页面，以及工作包 D 的产品模板和设备接入页面；A、B 新增结构与 C、D 菜单迁移已通过隔离 MySQL 8.0 验证。工作包 E 已完成目标云基础部署和合成单包转换核对，但真实设备、目标业务库和长时间运行仍未验收，候选也尚未批准为新的正式版本。当前权威基线仍是既有已合并 V1 能力。
...
2. 连接真实设备，确认正式单位、累计/周期语义、发送周期、时间戳和序号，并通过新的待绑定与接入流程完成单包核对。
3. 在真实单包和本地正式链确认后，执行 24 小时运行、断线恢复和数据质量验收。
...
| [`docs/designs/2026-08-17-configurable-device-onboarding-design.md`](docs/designs/2026-08-17-configurable-device-onboarding-design.md) | 活动候选设计、工作包 A/B/C/D 已实现、E 部分完成 | 已明确既有约束的保留与替代、云端适配边界、本地发现与绑定、页面范围和分段验收；E 仅完成目标云基础部署和合成单包，候选也未批准为正式基线。 |
