...
- 候选身份：该设计尚未实现、尚未验证、尚未独立验收，也未批准为新的正式版本；当前权威基线仍是本文件其余章节描述的已合并 V1 能力。
...
- 产品型号、产品测点模板、待绑定设备有界样例、接入向导以及建筑/空间/系统/设备/测点业务管理页面仍是活动候选设计，当前代码尚未实现。
...
1. 按[可配置设备接入与业务绑定设计](docs/designs/2026-08-17-configurable-device-onboarding-design.md)分工作包实现产品模板、待绑定设备、绑定事务和管理页面；绑定前仍不得写正式时序数据。
...
| [`docs/designs/2026-08-17-configurable-device-onboarding-design.md`](docs/designs/2026-08-17-configurable-device-onboarding-design.md) | 活动候选设计 | 已明确既有约束的保留与替代、云端适配边界、本地发现与绑定、页面范围和分段验收；当前尚未实现或批准为正式基线。 |
