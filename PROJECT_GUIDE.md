...
本文件只描述稳定定位、架构和职责。开始任务时先读取 [`AGENTS.md`](AGENTS.md)，再按任务需要读取本文件相关章节：

- 涉及定位、架构、模块、数据链路或运行入口时，读取对应章节；
- 涉及当前能力、阶段、风险或下一步时，只读取 [`PROJECT_STATUS.md`](PROJECT_STATUS.md) 的相关章节；
- 涉及具体实现时，读取直接相关的代码、测试和专题文档。

普通单点任务不要求完整读取本文件或 `PROJECT_STATUS.md`。
