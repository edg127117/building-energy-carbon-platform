...
温度模板接入候选位于 `iot.temperature`，设计见[自动匹配与人工模板计划](docs/designs/2026-09-25-hvac-temperature-template-onboarding-plan.md)。新页面默认 `AUTO`，允许 `MANUAL` 和 `STATE_ONLY`，旧请求省略模式保持原意。自动与人工使用服务端同一预览摘要及建点逻辑；首次配置需启用双温度模板草稿并审批建筑/厂家来源匹配规则。新设备计划随原接入申请执行；已绑定设备经 `/api/v1/operations/hvac-temperature-bindings` 的预览、批量任务和逐设备审批补齐，保留资产产品归属与身份启停。V63 保存规则、绑定快照、任务及明细，不保存温度历史。提交后缓存刷新失败可重试；审批期间配置变化必须重新预览。多厂家通过 `TemperatureAdapter` 描述可信协议、语义、单位及来源，当前仅大金内机有生产实现；目录没有可信型号时不按设备名称猜型号。页面重开可恢复本人最近的设备批量任务。采样进度与配置完成分别报告，只有核对到检查点对应历史且历史质量策略允许时才显示可查询；曲线继续经原质量策略与 TDengine 查询。
