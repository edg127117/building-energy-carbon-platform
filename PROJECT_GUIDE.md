...
大金只读接入位于 `com.platform.iot.daikin`：`client` 负责认证、只读路径和受限 HTTP 传输，`mapping` 规范化内外机字段，`catalog` 校验完整目录，`onboarding` 将可信完整目录登记至既有待接入区，并维护来源、项目建筑映射及版本。来源注册不包含凭据或目标 URL；设备读取客户端须显式启用，仅装配已联调的认证、内机及外机清单封装。完整目录入口是内部应用契约，调用者须先完成分页/总数校验并提供可靠轮次时间；`sync` 将目录读取与入库串接为持久化后台任务，不接收浏览器上传厂家目录。

设备接入继续复用 `DeviceOnboardingService`、台账与公共审批。大金状态型产品和绑定通过增量契约支持零数值测点，旧数值绑定规则保留；绑定完成的身份仍停用，启用另走既有审批，身份配置生效不代表正式数据已采集。`/api/v1/daikin/directory` 提供管理员来源登记和项目建筑映射，`/api/v1/device-products/typed-state` 创建状态产品草稿，`/api/v1/operations/device-onboarding` 提供菜单及建筑范围受控的厂家待接入查询、状态维护和逐设备幂等绑定申请；审核执行进入统一敏感变更申请页面。V47/V48 新增目录结构、外机类型和内外机状态产品草稿，不迁移菜单、不自动启用产品，不写温度/状态历史。后续实施边界见[只读接入设计](docs/designs/2026-09-15-daikin-readonly-integration-design.md)，完成状态以 `PROJECT_STATUS.md` 为准。
...
同步配置前缀为 `daikin.directory-sync`：`enabled=false`，任务扫描默认 10 秒，来源目录刷新默认 3600 秒，租约默认 120 秒，最多尝试 3 次，基础退避 30 秒，每批最多执行 4 个任务；均可在受限范围内配置，实际频率和租约需结合厂家限流与单页耗时调整。每页请求前核验并续租，过期任务不能续活；多节点部署须同步系统时钟。生产设备客户端配置前缀为 `daikin.device-client`，默认关闭；须从受保护的服务端环境提供 `source-id`、`base-uri`、`app-id`、`password`、`communication-key`、`signature-salt`，并显式设置 `enabled=true`。`source-id` 必须与管理员登记的稳定来源一致；凭据和地址不从浏览器传入，也不写入仓库。目录同步与分钟监测各自独立启用，启用客户端本身不会发出请求；没有可信 provider 时调度不发出外部请求。运行时长不属于该设备读取客户端。
...
内机类型化产品可显式提供 `temperatureTemplates`（仅 `roomTemp`、`temperature`，单位 `°C`、不参与冷站计算）；绑定申请增加可选 `pointBindings` 与同建筑启用 HTTP 数值来源 `numericSourceId`，别名命名空间固定为 `DAIKIN_V2`。旧零点调用保持兼容，外机仍可零点。本项目厂家协议分别定义室内温度和设定温度，同项目大金界面均以 °C 显示；生产设备客户端据此默认按 °C 解码，`daikin.device-client.temperature-celsius-confirmed=false` 可在发现现场字段不符时关闭。正式测点未绑定或配置不匹配时仍不写入有效温度；已绑定零点产品不静默升级。

温度模板接入候选位于 `iot.temperature`，设计见[自动匹配与人工模板计划](docs/designs/2026-09-25-hvac-temperature-template-onboarding-plan.md)。新页面默认 `AUTO`，允许 `MANUAL` 和 `STATE_ONLY`，旧请求省略模式保持原意。自动与人工使用服务端同一预览摘要及建点逻辑；首次配置需启用双温度模板草稿；没有 HTTP 数值来源时，可由管理员对同建筑、同厂家来源的已绑定内机一次预览和提交初始化配置包，经一张审批单原子建立来源、首批双温度测点及采集策略、来源专用匹配规则。已有来源继续使用规则配置和普通批量补齐；初始化不覆盖已有来源或规则。新设备计划随原接入申请执行；已绑定设备经 `/api/v1/operations/hvac-temperature-bindings` 的预览、批量任务和逐设备审批补齐，保留资产产品归属与身份启停。V63 保存规则、绑定快照、任务及明细，不保存温度历史。提交后缓存刷新失败可重试；审批期间配置变化必须重新预览。多厂家通过 `TemperatureAdapter` 描述可信协议、语义、单位及来源，当前仅大金内机有生产实现；目录没有可信型号时不按设备名称猜型号。页面重开可恢复本人最近的设备批量任务。采样进度与配置完成分别报告，只有核对到检查点对应历史且历史质量策略允许时才显示可查询；曲线继续经原质量策略与 TDengine 查询。
...
平台自算开机时长与上述厂家统计分开：分钟监测收到相邻有效 `onOff` 观测时，将不超过 5 分钟的间隔归于前一次开关状态，按北京时间拆分并累计每日开机时长和有效观测覆盖时长；缺失、无效、跨归属及长间断不补算。`/api/v1/hvac-monitoring/devices/{equipmentId}/observed-runtime` 按日、月、年汇总已观测数据并返回覆盖时长，页面明确标为“平台开机时长”，不宣称厂家等效运行时间或接入前历史。明细按 365 天窗口查询和清理，仍按当前正式设备身份、建筑和项目映射授权。
...
接入页面显示为“设备报文接入”，使用规范路由 `/configuration/ingestion/protocols`，按产品、映射、解析检查、发布加载、设备接入分阶段操作。
...
V58 将旧 HVAC 设备、19 测点、别名、指标、采集与质量治理策略、关联审核快照及菜单复制到独立的 `iot_platform_archive`；V59 逐表确认归档覆盖后才从活动库退役确定性旧基线。发现自定义别名、策略修订或业务引用时迁移停止，必须先单独复核。归档库不参与应用查询，部署账号需具备创建和写入该归档库的权限。
...
平台恢复会话时重新向后端核验身份和授权，不信任本地缓存角色。数据库菜单树决定工作区、分组、叶子顺序和显示名称；客户端只接受与本地注册表完全一致的规范叶子路径，并校验其工作区和父分组。目录不自动授予子页面，管理员角色也不自动扩权；旧路径不再由前端重定向或映射。迁入管理页面还需保留平台管理员角色检查；前端导航限制不能替代后端接口鉴权和建筑范围校验。菜单缓存使用 V2 命名空间，避免菜单升级后读取旧层级。

敏感变更申请统一位于“能碳配置管理平台 → 用户与权限 → 敏感变更申请”（`/configuration/access/changeRequests`）。V61 为已有角色授权该页面入口；我的申请只返回本人数据，审核和执行仍需后端 `BACKOFFICE_CHANGE_REVIEWER` 职责。进行中与历史申请分开分页，历史记录保留审计；批量审核并执行适用于待办中的设备绑定或身份启用申请，两类分开选择，逐项调用原审核与执行接口。大金已绑定设备可在待接入列表批量提交身份启用申请，逐台核对绑定状态并保留失败结果。

设备台账、设备列表及详情位于“智慧运维 → 设备管理 → 设备台账”（`/operations/devices/businessDevices`），保留资产接口的平台管理员要求。
...
- 智慧运维 → 设备管理 → 待接入设备：`/operations/devices/pendingDevices`。V59 保留叶子菜单 ID 与角色授权并收口到规范父子树；厂家配置、凭据及产品模板继续留在配置侧。
