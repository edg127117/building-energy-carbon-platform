...
敏感变更申请统一位于“能碳配置管理平台 → 用户与权限 → 敏感变更申请”（`/configuration/access/changeRequests`）。V61 为已有角色授权该页面入口；我的申请只返回本人数据，审核和执行仍需后端 `BACKOFFICE_CHANGE_REVIEWER` 职责。进行中与历史申请分开分页，历史记录保留审计；批量审核并执行仅适用于待办中的设备绑定申请，逐项调用原审核与执行接口。
